...
- [Mnemosyne](https://github.com/ElonAug7/Mnemosyne-agentmemory-engine-openclaw-hermes) by [ElonAug7](https://github.com/ElonAug7) — Zero-dependency local-first cognitive memory engine: compound-cue retrieval grounded in 31 cognitive-science papers, true BM25 ranking plus a pure-local semantic dictionary (no LLM, no embedding API). Native Hermes MemoryProvider bridge + skill, OpenClaw gateway hook, built-in Web UI, <50ms search. Beats every embedding system on the Memory-Native benchmark (nDCG@10 0.238). **[beta]**
