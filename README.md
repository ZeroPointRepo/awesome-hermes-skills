...
- [nexus-memory](https://github.com/Neboy72/nexus-memory) by [Neboy72](https://github.com/Neboy72) — Self-hosted memory provider with BM25+vector+RRF hybrid retrieval, cross-encoder reranking, and Memory Dynamics (recall reinforcement, linear decay, salience immunity). Zero API keys to start: local embeddings + reranker free out of the box, bge-m3 upgrade path. Hermes plugin, OpenClaw plugin, or MCP server on one Qdrant store. 726 tests. Apache-2.0. **[beta]**
